...
If building from source without yet a launcher script, do the build then
make a helpful alias like::
  ...
  $ rm -fr "$TARGET_DIR"
  $ mkdir "$TARGET_DIR"
  $ alias download="java -jar \"$SOURCE_DIR\"/target/downloader-*-jar-with-dependencies.jar -d \"$TARGET_DIR\" -s eel.openmicroscopy.org -u user-3 -w ome"
  $ cd
...
#. Download the image file using the session key::
    ...
    java -jar "$SOURCE_DIR"/target/downloader-*-jar-with-dependencies.jar -d "$TARGET_DIR" -s eel.openmicroscopy.org -k $KEY -f binary $DICOM_IMAGE
