...
the Jenkins CI. Staging documentation is deployed to `Read the Docs`
via a GitHub Action that run on the PR.
...
Hosted at `<https://ome-contributing.readthedocs.io/>`_ (always latest).
...
The source files are in `<https://github.com/ome/ome-model-documentation>`_.
...
Hosted at `<https://omero.readthedocs.io/en/>`_\{{version}}/ (plus
latest redirect - :omero_doc:`https://omero.readthedocs.io/en/stable/ <>`).
